...
data = {'asics': asics, 'on': on, 'brooks': brooks, 'prices': {'asics': asics_price, 'on': on_price}}
body = (
...
print('barcodes  asics: %d  on: %d  brooks: %d   |   prices  asics: %d  on: %d   ->  barcode-data.js (%.1f KB)' % (
    len(asics), len(on), len(brooks), len(asics_price), len(on_price), os.path.getsize(OUT) / 1024))
