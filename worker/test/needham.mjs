...
const jsonl = [
  ...
  { id: 'gid://V/1', sku: '1111-BLK-9', price: '140.00', inventoryItem: { id: 'gid://I/1' }, __parentId: 'gid://P/1' },
  { location: { id: NEEDHAM }, quantities: [{ name: 'on_hand', quantity: 3 }], __parentId: 'gid://V/1' },
  { id: 'gid://V/2', sku: '1111-BLK-10', price: '140.00', inventoryItem: { id: 'gid://I/2' }, __parentId: 'gid://P/1' },
  ...
  { id: 'gid://V/3', sku: '2222-WHT-9', price: '160.00', inventoryItem: { id: 'gid://I/3' }, __parentId: 'gid://P/2' },
  ...
  { id: 'gid://V/4', sku: 'HAT', price: '20.00', inventoryItem: { id: 'gid://I/4' }, __parentId: 'gid://P/3' },
  { location: { id: NEEDHAM }, quantities: [{ name: 'on_hand', quantity: 9 }], __parentId: 'gid://V/4' },
];

// --- reassembly -------------------------------------------------------------
console.log('\nBulk reassembly (footwear only, Needham on-hand)');
const { raw, needhamOnHandMap } = assembleBulkObjects(jsonl, NEEDHAM);
...
check('Needham on-hand summed across variants (3 + 2)', needhamOnHandMap.get('clifton-9-black') === 5);
check('product stocked only elsewhere is NOT in the Needham map', !needhamOnHandMap.has('retail-only-shoe'));
check('the non-footwear hat at Needham is ignored', !needhamOnHandMap.has('a-hat'));

// --- payload, scoping ON ----------------------------------------------------
console.log('\nbuildCatalogFrom with Needham scoping ON');
const scoped = buildCatalogFrom(raw, needhamOnHandMap, { needhamLocationId: NEEDHAM, shop: 'test.myshopify.com' });
...
const { raw: raw2, needhamOnHandMap: map2 } = assembleBulkObjects(jsonl, null);
check('no Needham map when scoping off', map2 === null);
const unscoped = buildCatalogFrom(raw2, map2, { needhamLocationId: null, shop: 'test.myshopify.com' });
